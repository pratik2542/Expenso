...
// Component to display amount in its original currency with +/- and color coding
...
export default function Expenses() {
  ...
  useEffect(() => {
    if (router.query.action === 'add') {
      ...
      router.replace('/expenses', undefined, { shallow: true })
    } else if (router.query.account) {
      // Filter by account
      setAccountFilter(router.query.account as string)
      if (router.query.accountName) {
        setAccountName(decodeURIComponent(router.query.accountName as string))
      }
    }
  }, [router.query.action, router.query.account, router.query.accountName])
  ...
  const formatCompact = (amount: number, currencyCode: string) => {
    const absAmount = Math.abs(amount)
    const sign = amount < 0 ? '-' : ''
    let symbol = ''
    try {
      symbol = new Intl.NumberFormat(undefined, { style: 'currency', currency: currencyCode }).format(0).replace(/[\d.,\s]/g, '')
    } catch { symbol = currencyCode + ' ' }
    ...
    // For smaller amounts, use regular explicit format but ensure it's not too long
  ...
  const filteredExpenses = expenses.filter(expense => {
    ...
    const mappedCategory = normalizeCategory(expense.category, definedCategoryNames, expense.type)
    ...
    // Account filter - include transactions where the account is either the source OR destination
    const matchesAccount = !accountFilter ||
      expense.account_id === accountFilter ||
      expense.toAccountId === accountFilter
  ...
  const totalAmount = sortedExpenses.reduce((sum, expense) => {
    if (accountFilter && expense.type === 'transfer') {
      // For transfers, check which account we're viewing
      if (expense.account_id === accountFilter) {
        // Transfer FROM this account - subtract
        return sum - (expense.transferAmount || 0)
      } else if (expense.toAccountId === accountFilter) {
        // Transfer TO this account - add
        return sum + (expense.transferAmount || 0)
      }
    }
  ...
  // No automatic conversion anymore. Multi-currency environments show distinct totals.
  useEffect(() => {
  ...
  const categoryAvgPerMonth = (() => {
    ...
    const avgPerMonth = Math.abs(totalAmount / monthlyData.size)
  ...
  const categoryPredictedYearly = (() => {
    ...
    const predictedMonthly = Math.abs(totalAmount / 12)
  ...
  const deleteExpense = async (id: string) => {
    ...
    setDeletingId(id)
    try {
      // Find the expense to get its details before deleting
  ...
  const handleAttachBill = async (file: File) => {
    ...
    try {
      ...
      const expenseDocRef = doc(getCollection('expenses'), attachingToExpense.id)
      ...
      <RequireAuth>
          ...
          <div className="max-w-7xl mx-auto px-4 lg:px-8 py-4 lg:py-8">
            ...
            <div className="mb-4 lg:mb-8">
              ...
              <div className="lg:hidden">
                {/* Top Row: Logo/Title and Search Icon */}
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <h1 className="text-xl font-bold text-gray-900 dark:text-white">Expenses</h1>
                    {accountName && (
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Account: {accountName}</p>
                    )}
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="w-10 h-10 flex items-center justify-center text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
                  >
                ...
                {accountFilter && accountName && (
                  <div className="mb-3 flex items-center gap-2">
                    <div className="flex-1 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg px-3 py-2 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Wallet className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                        <span className="text-sm font-medium text-blue-700 dark:text-blue-300">{accountName}</span>
                      </div>
                      <button
                        onClick={() => {
                          setAccountFilter('')
                          setAccountName('')
                          router.push('/expenses', undefined, { shallow: true })
                        }}
                ...
                <div className="grid grid-cols-5 gap-2 mb-3">
                  ...
                  <button
                    className="flex flex-col items-center justify-center py-2 text-xs font-medium rounded-xl bg-amber-50 text-amber-700 border border-amber-200 hover:bg-amber-100 transition-all"
                    onClick={() => {
                ...
                <div className="flex items-center justify-between mb-3">
                  <button
                    ...
                    disabled={!monthFilter || availableMonths.indexOf(monthFilter) >= availableMonths.length - 1}
                    className="w-8 h-8 flex items-center justify-center text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                  >
                  ...
                  <div className="flex-1 text-center">
                    ...
                    >
                      {monthFilter ? (() => {
                        ...
                        return new Date(parseInt(year), parseInt(monthNum) - 1).toLocaleString('default', { month: 'long', year: 'numeric' })
                      })() : 'All Time'}
                    </button>
                  </div>
                  ...
                  <button
                    onClick={() => {
                      const currentIndex = availableMonths.indexOf(monthFilter)
                      if (monthFilter && currentIndex > 0) {
                        setMonthFilter(availableMonths[currentIndex - 1])
                      } else if (!monthFilter && availableMonths.length > 0) {
                        setMonthFilter(availableMonths[0])
                      }
                    }}
                    disabled={!!(monthFilter && availableMonths.indexOf(monthFilter) === 0)}
                    className="w-8 h-8 flex items-center justify-center text-gray-600 hover:bg-gray-100 rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <button
                    ...
                    className="w-8 h-8 ml-2 flex items-center justify-center text-gray-600 hover:bg-gray-100 rounded-lg transition-colors"
                  >
              ...
              <div className="hidden lg:block">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-400 mt-1 sm:mt-2 text-sm sm:text-base">Track and manage all your expenses</p>
                  </div>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          // If no duplicates found, that's fine - just leave arrays empty
                        } catch (e: any) {
                          setDuplicatesError(e.message || 'Error detecting duplicates')
                        } finally {
                          setDuplicatesLoading(false)
            ...
            {/* Account Filter Badge - Desktop */}
            {accountFilter && accountName && (
              <div className="hidden lg:block mb-4">
                <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-xl px-4 py-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Wallet className="w-5 h-5 text-blue-600 dark:text-blue-400" />
                    <span className="text-sm font-medium text-blue-700 dark:text-blue-300">Viewing transactions for: {accountName}</span>
                  </div>
                  <button
                    onClick={() => {
                      setAccountFilter('')
                    ...
                    className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-200 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-lg px-3 py-1.5 text-sm font-medium transition-colors"
                  >
                    Clear Filter
                  </button>
                </div>
            ...
            <div id="filters-section" className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-3 lg:card lg:!p-6 mb-4 lg:mb-6">
              {/* Mobile: Hidden search input for focus target */}
              <div className="lg:hidden space-y-2.5">
                <div className="relative">
                  <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 w-4 h-4" />
                  <input
                    type="text"
                ...
                {categoryFilter && (
                  <div className="space-y-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-gray-500 dark:text-gray-400">Filtered by:</span>
                      <span className="font-semibold text-primary-600 dark:text-primary-400">{categoryFilter}</span>
                    </div>
                    {categoryAvgPerMonth && (
                      <div className="flex items-center justify-between text-xs">
              ...
              <div className="hidden lg:block">
                <div className="flex flex-col gap-2 sm:flex-row sm:gap-4">
                  <div className="flex flex-col gap-2 sm:flex-row sm:gap-4 w-full">
                    <div className="flex-1">
                      ...
                      <div className="relative">
                        <SearchIcon className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 w-4 h-4 sm:w-5 sm:h-5 pointer-events-none" />
                        <input
                          type="text"
                          placeholder="Search..."
                          value={searchTerm}
                          onChange={(e) => setSearchTerm(e.target.value)}
                          className="input !pl-10 sm:!pl-14 pr-3 dark:bg-gray-700 dark:text-white dark:border-gray-600 dark:placeholder:text-gray-400"
                        />
                      </div>
                    </div>
                    <div className="sm:min-w-[150px]">
                      <label className="block text-xs font-medium text-gray-600 mb-1 sm:sr-only">Type</label>
                      <select
                        value={typeFilter}
                    ...
                    <div className="sm:min-w-[180px]">
                      ...
                      >
                        ...
                        {[...new Set([...categories.map(c => c.name), 'Transfer'])]
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 sm:py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                ...
                {/* Results Summary */}
                <div className="mt-2 pt-2 sm:mt-4 sm:pt-4 border-t border-gray-200 dark:border-gray-700 flex flex-col gap-2 sm:gap-3">
                  ...
                  {categoryPredictedYearly && (
                    <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-3 text-sm">
                      <span className="text-gray-600 dark:text-gray-400">Predicted monthly average (yearly):</span>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-4 lg:card lg:!p-6">
              {distinctCurrencies.size > 1 && (
                <div className="mb-4 p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 text-sm flex flex-wrap gap-2 lg:gap-4">
                  <div className="font-medium text-gray-700 dark:text-gray-300 w-full text-xs lg:text-sm">Per-currency totals:</div>
                  {perCurrencyTotals.map(c => (
                    <div key={c.code} className="px-2 py-1 bg-white dark:bg-gray-600 border border-gray-200 dark:border-gray-500 rounded shadow-sm text-gray-800 dark:text-gray-200">
                      {c.code}: {c.total.toFixed(2)}
                    </div>
                  ))}
                  <div className="text-xs text-gray-500 dark:text-gray-400 w-full">Add FX conversion to show unified total.</div>
                </div>
              )}
              {actionError && (
                <div className="mb-3 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-3">{actionError}</div>
              )}
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                      {!isLoading && sortedExpenses.map((expense) => {
                        ...
                        return (
                          <div
                            key={expense.id}
                            className="p-4 bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 last:border-b-0 active:bg-gray-50 dark:active:bg-gray-700 transition-colors cursor-pointer"
                            onClick={() => setSelectedExpense(expense)}
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex-1 min-w-0">
                                <h3 className="text-sm font-medium text-gray-900 dark:text-white mb-0.5 line-clamp-2">
                              ...
                              <div className="flex flex-col items-end gap-1">
                                <span className="text-base font-semibold text-gray-900 dark:text-white">
                                  <ConvertedAmount
                                    ...
                                    type={expense.type}
                                    transferAmount={expense.transferAmount}
                                  />
                                </span>
                          ...
                          <tr key={expense.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td className="px-6 py-4">
                              <div>
                                ...
                                {expense.merchant && (
                                  <div className="text-sm text-gray-500 dark:text-gray-400 break-words line-clamp-2">
                                    {expense.merchant}
                                  </div>
                                )}
                              </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {(['', 'expense', 'income', 'transfer'] as const).map((val) => {
        ...
        {showDuplicatesModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-2 sm:p-4">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl relative max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col">
              {/* Gradient Header */}
                ...
                {duplicatesLoading ? (
                  ...
                  <div className="py-12 flex flex-col items-center text-center">
                    <div className="w-16 h-16 rounded-2xl bg-green-100 dark:bg-green-900/30 flex items-center justify-center mb-4">
                      <svg className="w-8 h-8 text-green-500 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    </div>
                    <p className="text-gray-900 dark:text-white font-semibold mb-1">All Clear!</p>
                    <p className="text-gray-500 dark:text-gray-400 text-sm">No duplicates detected in your expenses</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    ...
                    {duplicateGroups.map((group, idx) => {
                      ...
                      return (
                        <div key={idx} className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden bg-white dark:bg-gray-700">
                          {/* Group Header */}
                          <div className="bg-gradient-to-r from-gray-50 to-slate-50 dark:from-gray-800 dark:to-gray-700 px-4 py-3 border-b border-gray-100 dark:border-gray-600">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                              <div className="flex items-center gap-2">
                                <span className="text-sm font-semibold text-gray-800 dark:text-white">Group {idx + 1}</span>
                                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${group.confidence === 'high' ? 'bg-green-100 text-green-700' :
                            ...
                            <p className="text-xs text-gray-600 dark:text-gray-400 mt-1.5 line-clamp-2">{group.reason}</p>
                          </div>
                          ...
                          <div className="divide-y divide-gray-100 dark:divide-gray-600">
                            ...
                            {group.original_id && (() => {
                              ...
                              return (
                                <div className="px-4 py-3 bg-green-50/50 dark:bg-green-900/20 flex items-center gap-3">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-2">
                                      <span className="font-medium text-gray-900 dark:text-white text-sm truncate">{original.note || original.merchant || original.category}</span>
                                      <span className="text-sm font-semibold text-gray-900 dark:text-white flex-shrink-0">{original.amount} {original.currency}</span>
                                    </div>
                                    <div className="flex items-center gap-2 mt-0.5">
                                      <span className="text-xs text-green-600 dark:text-green-400 font-medium">Keep</span>
                                      <span className="text-xs text-gray-400 dark:text-gray-500">•</span>
                                      <span className="text-xs text-gray-500 dark:text-gray-400">{formatDate(original.occurred_on)}</span>
                                    </div>
                                  </div>
                                </div>
                              )
                            ...
                            {group.duplicate_ids.map((id: string) => {
                              ...
                              return (
                                <div
                                  key={id}
                                  className={`px-4 py-3 flex items-center gap-3 cursor-pointer transition-colors ${isSelected ? 'bg-red-50/50 dark:bg-red-900/20' : 'hover:bg-gray-50 dark:hover:bg-gray-600'
                                    }`}
                                  onClick={() => {
                                ...
                                >
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-2">
                                      <span className={`font-medium text-sm truncate ${isSelected ? 'text-red-700 dark:text-red-400' : 'text-gray-900 dark:text-white'}`}>
                                        {exp.note || exp.merchant || exp.category}
                                      </span>
                                      <span className="text-sm font-semibold text-gray-900 dark:text-white flex-shrink-0">{exp.amount} {exp.currency}</span>
                                    </div>
                                    <div className="flex items-center gap-2 mt-0.5">
                                      <span className={`text-xs font-medium ${isSelected ? 'text-red-500 dark:text-red-400' : 'text-gray-400 dark:text-gray-500'}`}>
                                        {isSelected ? 'Delete' : 'Keep'}
                                      </span>
                                      <span className="text-xs text-gray-400 dark:text-gray-500">•</span>
                                      <span className="text-xs text-gray-500 dark:text-gray-400">{formatDate(exp.occurred_on)}</span>
                                    </div>
                                  </div>
                                </div>
                              )
              ...
              {!duplicatesLoading && (duplicateGroups.length > 0 || detectedDuplicates.length > 0) && (
                <div className="border-t border-gray-200 dark:border-gray-700 px-4 py-4 sm:px-6 bg-gray-50 dark:bg-gray-700 flex flex-col sm:flex-row gap-2 sm:justify-end">
                  <button
                    className="px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors w-full sm:w-auto"
                    onClick={() => setShowDuplicatesModal(false)}
                  >Cancel</button>
                  <button
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6">
                {similarGroups.length === 0 ? (
                  <div className="py-12 flex flex-col items-center text-center">
                    ...
                    <p className="text-gray-900 dark:text-white font-semibold mb-1">All Unique!</p>
                    <p className="text-gray-500 dark:text-gray-400 text-sm">No similar expenses found in the same month</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {/* Summary Badge */}
                    ...
                    {similarGroups.map((group, groupIdx) => {
                      ...
                      return (
                        <div key={groupIdx} className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden bg-white dark:bg-gray-700">
                          {/* Group Header */}
                          <div className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 px-4 py-3 border-b border-amber-100 dark:border-amber-800 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <input
                                type="checkbox"
                              ...
                              />
                              <div>
                                <span className="font-semibold text-gray-800 dark:text-white text-sm">{monthName}</span>
                                <span className="text-xs text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/30 px-2 py-0.5 rounded-full ml-2">
                          ...
                          <div className="divide-y divide-gray-100 dark:divide-gray-600">
                            {group.expenses.map(exp => {
                              ...
                              return (
                                <div
                                  key={exp.id}
                                  className={`px-4 py-3 flex items-center gap-3 cursor-pointer transition-colors ${isSelected ? 'bg-amber-50/50 dark:bg-amber-900/20' : 'hover:bg-gray-50 dark:hover:bg-gray-600'
                                    }`}
                                  onClick={() => {
                                    setSelectedSimilarIds(prev => {
                                      const next = new Set(prev)
                                ...
                                >
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5">
                                      {exp.merchant && <span className="text-xs text-gray-500 dark:text-gray-400 truncate">{exp.merchant}</span>}
                                      {exp.merchant && <span className="text-xs text-gray-300 dark:text-gray-600">•</span>}
                                      <span className="text-xs text-gray-500 dark:text-gray-400">{formatDate(exp.occurred_on)}</span>
                                    </div>
                                  </div>
                                </div>
        ...
        {actionOpenId && actionExpense && menuPos && (
          <>
            ...
            />
            <div
              className="fixed z-50 w-40 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg flex flex-col py-1"
              style={{ top: menuPos.top, left: menuPos.left }}
              role="menu"
            >
              <button
                className="block w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 dark:text-gray-200"
                onClick={() => {
              ...
              <button
                className="block w-full text-left px-3 py-2 text-sm hover:bg-red-50 dark:hover:bg-red-900/30 text-red-600 dark:text-red-400"
        ...
        {selectedExpense && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black bg-opacity-50" onClick={() => setSelectedExpense(null)}>
            <div
              className="bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-lg max-h-[85vh] overflow-auto"
              ...
              <div className="p-6 space-y-4" style={{ paddingBottom: 'max(calc(3rem + env(safe-area-inset-bottom, 0px)), 3rem)' }}>
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-gray-900 dark:text-white">
                      <ConvertedAmount
                        ...
                        transferAmount={selectedExpense.transferAmount}
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1">
                      <span className={categoryBadgeClass(normalizeCategory(selectedExpense.category, definedCategoryNames, selectedExpense.type))}>
                        {normalizeCategory(selectedExpense.category, definedCategoryNames, selectedExpense.type)}
                      </span>
                    </p>
                  </div>
